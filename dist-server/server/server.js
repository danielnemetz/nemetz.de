...
import { DEFAULT_LANG, SUPPORTED_LANGS, buildLocalizedPath, getBasePathForKey, getKeyByPath, splitLocalizedPath, } from '../src/lib/routing.js';
...
const localeDir = path.join(rootDir, isDev ? path.join('public', 'i18n') : path.join('dist', 'i18n'));
...
function isModalKey(value) {
    return value === 'about' || value === 'imprint' || value === 'privacy';
}
async function loadTemplateString() {
    ...
    document.querySelectorAll('.lang-toggle').forEach((link) => {
        ...
        if (isLang(lang)) {
            ...
            link.setAttribute('aria-current', `<%= ${JSON.stringify(lang)} === it.lang ? 'page' : 'false' %>`);
        }
    });
    ...
    template = template.replace(/ data-open-placeholder="(about|imprint|privacy)"/g, (_match, key) => {
    ...
    template = template.replace(/<\/body>/, `<% if (it.initialDialog) { %><div class="dialog-backdrop" data-backdrop-for="<%= it.MODALS[it.initialDialog].id %>"></div><% } %>\n    </body>`);
    template = template
        .replace(/&lt;%/g, '<%')
        .replace(/%&gt;/g, '%>')
        .replace(/&quot;/g, '"')
...
async function renderPage({ lang, dialog, basePath, }) {
    const template = await getTemplate();
    const locale = await loadLocale(lang);
    const fallback = lang === DEFAULT_LANG ? locale : await loadLocale(DEFAULT_LANG);
    const translator = (key) => getTranslationValue(locale, key) ??
        getTranslationValue(fallback, key) ??
        '';
    const context = {
        lang,
...
async function render404Page(lang) {
    const template = await getTemplate();
    const locale = await loadLocale(lang);
    const fallback = lang === DEFAULT_LANG ? locale : await loadLocale(DEFAULT_LANG);
    const translator = (key) => getTranslationValue(locale, key) ??
    ...
    document.querySelectorAll('.lang-toggle').forEach((link) => {
        ...
        if (isLang(linkLang)) {
            ...
            link.setAttribute('aria-current', `<%= ${JSON.stringify(linkLang)} === it.lang ? 'page' : 'false' %>`);
        }
    });
    // Note: data-i18n elements in main are already handled by Eta template
    const templateString = document.toString()
        .replace(/&lt;%/g, '<%')
        .replace(/%&gt;/g, '%>')
        .replace(/&quot;/g, '"')
        .replace(/&apos;/g, "'")
...
function determineLang(pathname, searchParams) {
    ...
    fastify.setNotFoundHandler(async (request, reply) => {
        ...
        if (basePath !== '/' && dialog === null) {
            ...
            return reply
                ...
                .header('Cache-Control', isDev ? 'no-store' : 'public, max-age=300, s-maxage=300, stale-while-revalidate=60')
                .send(html);
        }
