...
async function renderPage({
    ...
    const fallback =
        lang === DEFAULT_LANG ? locale : await loadLocale(DEFAULT_LANG);
...
function determineLang(
    ...
    const needsRedirect =
        pathname !== canonicalPath || sanitizedString !== searchParams.toString();
...
async function start(): Promise<void> {
    ...
    fastify.setNotFoundHandler(async (request, reply) => {
        ...
        const { lang, basePath, needsRedirect, search } = determineLang(
            url.pathname,
            url.searchParams,
        );
